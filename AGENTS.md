...
- `src/sound.ts` - Web Audio `Synth` (per held note one oscillator, or a detuned stereo unison stack, into an ADSR envelope; shared lowpass/volume, generated convolution reverb, limiter) and the pure Sound-tab settings helpers. Off by default; browsers keep audio suspended until a click or key press (MIDI doesn't count), hence the top-bar unlock button.
